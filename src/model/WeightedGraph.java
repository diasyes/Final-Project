...
// Create a ShortestPathTree
     ...
     * Find single source shortest paths
     */
    public ShortestPathTree getShortestPathAlternative(int sourceVertex) {
        // cost[v] stores the cost of the path from v to the source
        double[] cost = new double[getSize()];
        for (int i = 0; i < cost.length; i++) {
            cost[i] = Double.POSITIVE_INFINITY; // Initial cost set to infinity
        ...
        while (T.size() < getSize()) {
            ...
            int u = -1; // Vertex to be determined
            double currentMinCost = Double.POSITIVE_INFINITY;
            for (Integer uCandidate : T) {
                for (Edge e : neighbors.get(uCandidate)) {
                    if (!T.contains(e.v) && currentMinCost > cost[uCandidate] + ((WeightedEdge) e).weight) {
                        currentMinCost = cost[uCandidate] + ((WeightedEdge) e).weight;
                        v = e.v;
                        u = uCandidate;
                    }
                }
